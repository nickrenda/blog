...
\label{eq:def2}
\end{equation}
*for all $\lambda \in \Lambda$ whenever $A \in \mathcal{B}(\mathbb{R})$ such that 
$\mu(A) < \delta$.*

**Proof:** \
(Definition 1 >> Definition 2) Let $\epsilon > 0$. Choose $T > 0$ such that
$\sup_{\lambda}\int\ |f_{\lambda}|\cdot I(|f_{\lambda}| > T)\ d\mu < \epsilon / 2$. 
Then let $\delta := \epsilon / 2T$. Thus, for $A \in \mathcal{B}(\mathbb{R})$ with 
$\mu(A) < \delta$,
...
for all $\lambda \in \Lambda$. $\blacksquare$ (>>)
...
$\mu(\{|f_{\lambda_n}| > n\}) \geq \epsilon$. But then by we can choose $\{E_{n}\}_{n=0}^{\infty}$ such 
that $E_n \in \mathcal{B}(\mathbb{R})$,  $\mu(E_n) = \delta / 2$, and $|f_{\lambda_n}(x)| > n$ for all $x \in E_n$ (* see notes below). But then if we choose $N$ such that 
...
\* One property of Lebesgue measure is that if $\mu(A) = R > 0$, then for all $0 < r < R$, 
there exists a measurable $E \subset A$  such that $\mu(E) = r$. 
